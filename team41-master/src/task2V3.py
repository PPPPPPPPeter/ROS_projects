...
class wall_follower:
    ...
    def main(self):
        ...
        while not self.ctrl_c:
            ...
            rospy.loginfo(f'The state is {state}')
            if (state == States.WALL_FOLLOW):
                ...
                self.robot_controller.set_move_cmd(0.24, -0.7) 

            elif (state == States.TOleft or state == States.TURNleft):
                self.robot_controller.set_move_cmd(0.24, 1.7)
    ...
    def adjustalittle(self):
        self.adjustalittle_thre = 0.3
        if self.min_dis <= self.adjustalittle_thre:
