...
class ScanTwistCenterControlNode:
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():  # running until being interrupted manually
            continue
        self.cmd_vel_pub.publish(Twist())  # Stop robot and exist
    ...
    def _call_back(self, msg):
        ...
        read_from_idx = int(size * (self.direction + 1) / 4)  # 180-360 for direction 1 (LHR policy)
        read_to_idx = int(size * (self.direction + 3) / 4)  # 0-180 for direction -1 (RHR policy )
        print(read_from_idx)
        print(read_to_idx)
        ...
        self.distance_front = ranges[int(size / 2)]      # Get distance at front
